...
execute as @s[tag=spectator] if score #Game GameStatus matches 1 run gamemode spectator @s
execute as @s[tag=spectator] if score #Game GameStatus matches 1 run tp @s @r[tag=ingame,tag=!out]

execute as @s if score #Game GameStatus matches 0 run function yw-pillar:utils/player/join/before_game